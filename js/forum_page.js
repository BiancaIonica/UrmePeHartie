...
document.addEventListener("DOMContentLoaded", () => {
    ...
    onValue(postsRef, (snapshot) => {
      ...
      postsArray.reverse().forEach((postData) => {
        ...
        const isLiked =
          auth.currentUser &&
          postData.likedBy &&
          postData.likedBy[auth.currentUser.uid]
            ? "liked"
            : "";
        post.innerHTML = `
                    <p>${postData.text}</p>
                    <small>Posted by ${postData.user} on ${
          postData.timestamp
        }</small>
                    <div class="like-container">
                        <button class="like-btn ${isLiked}" data-category="${tabName.toLowerCase()}" data-post-id="${
          postData.id
        }">👍</button>
  ...
  function bindLikeButtons(category) {
    const likeButtons = document.querySelectorAll(
      `.like-btn[data-category="${category}"]`
    );
    ...
    get(postRef)
      .then((snapshot) => {
        ...
        set(postRef, postData)
          .then(() => {
            btn.nextElementSibling.textContent = postData.likes;
  ...
  ["Literature", "Poetry", "Chat"].forEach((tab) => {
    document
      .getElementById("post" + tab + "Button")
  ...
  document.getElementById("homeIcon").addEventListener("click", () => {
    window.location.href = "../index.html"; // sau calea către pagina principală
    });

  openTab(
    { currentTarget: document.querySelector(".tab-link.active") },
    "Literature"
  );
});
